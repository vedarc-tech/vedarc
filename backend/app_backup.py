...
def admin_generate_certificate():
    ...
    try:
        ...
        # Update user to mark certificate as generated
        if certificate_type == 'completion':
            users.update_one(
                {"user_id": user_id},
...
def save_certificate_template():
    ...
    try:
        ...
        required_fields = ['header_image', 'placeholders', 'orientation']
        for field in required_fields:
...
def upload_template_image():
    """Upload template header image (admin only)"""
    current_user = get_jwt_identity()
    if current_user != ADMIN_USERNAME:
        return jsonify({"error": "Unauthorized"}), 403
    ...
    try:
        ...
        if file:
            ...
            filename = f"template_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}_{secure_filename(file.filename)}"
            ...
            # Return the file path for template storage
            return jsonify({
                "success": True,
...
def generate_certificate_from_template(template_data, student_data, certificate_type):
    ...
    try:
        ...
        # Get template info
        header_image_path = template_data.get('header_image', '')
        ...
        if header_image_path:
            try:
                ...
                if header_image_path.startswith('/'):
                    img_path = os.path.join(os.getcwd(), header_image_path.lstrip('/'))
                else:
                ...
                if os.path.exists(img_path):
                    ...
                    c.drawImage(img_path, 0, height - new_height, width=width, height=new_height)
        ...
        for placeholder_id, placeholder_data in placeholders.items():
            ...
            font_size = placeholder_data.get('font_size', 12)
            font_name = placeholder_data.get('font_name', 'Helvetica')
            color = placeholder_data.get('color', '#000000')
            
            # Replace placeholders with actual data
            text = text.replace('{student_name}', student_data.get('fullName', ''))
            text = text.replace('{track_name}', student_data.get('track', ''))
            text = text.replace('{completion_date}', student_data.get('completion_date', ''))
            text = text.replace('{current_date}', datetime.utcnow().strftime('%d %B %Y'))
            text = text.replace('{manager_name}', student_data.get('manager_name', ''))
            text = text.replace('{company_name}', 'VEDARC TECHNOLOGIES')
...
def student_download_certificate(certificate_type):
    ...
    try:
        ...
        if certificate_type == 'completion':
            ...
            if not user.get('offer_letter_unlocked', False):
                return jsonify({"error": "Offer Letter is not unlocked yet"}), 403
